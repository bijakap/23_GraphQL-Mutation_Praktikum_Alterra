...
import { GetTodobyId, GetTodo, InsertTodo, UpdateTodo, DeteleTodo} from "./queries"



function TodoList() {
  const { data, loading, error} = useQuery(GetTodo);
  const [ getTodo, { data:dataById, loading:loadingByID, error:errorById, refretch }] = useLazyQuery(GetTodobyId);
  const [ insertTodo, {loading:insertLoading, error:errorInsert}] = useMutation(InsertTodo, {
    refetchQueries: [GetTodo]
  });
  const [ updateTodo, {loading:loadingUpdate, error:errorUpdate}] = useMutation(UpdateTodo, {
    refetchQueries: [GetTodo]
  })
  const [ deteleTodo, {loading:loadingDelete, error:errorDelete}] = useMutation(DeteleTodo, {
    refetchQueries: [GetTodo]
  })
  ...
  const onClickItem = (idx) => {
    updateTodo({
      variables:{
        'where' : {
          "id": {"_eq": data.todolist[idx].id}
        },
        '_set' : {
          "is_done": !data.todolist[idx].is_done
        }
      }
    })
  };
    ...
    <>
      <div className='container'>
        ...
        <ul className='todo-list js-todo-list'>
          { window === 'all' ?
            data?.todolist.map((todo, todoIdx) => (
              <Todo
                key={todoIdx}
                id={todoIdx}
                onClickItem={() => onClickItem(todoIdx)}
                onDeleteItem={() => onDeleteItem(todoIdx)}
                title={todo.title}
            ...
            <div>
              {
                dataById?.todolist.length == 0 ?
                <div>ID tidak ditemukan</div>
                :
                dataById?.todolist.map((todo, todoIdx) => (
                  <Todo
                    key={todoIdx}
                    id={todoIdx}
                    onClickItem={() => onClickItem(todoIdx)}
        ...
        <div className='searchByID'>
          <input value={ID} onChange={(e) => setID(e.target.value)} type='number' placeholder='Display With Spesific ID' />
          <button onClick={() => searchByID()}>Submit</button>
